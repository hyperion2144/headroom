...
# Install URL shown in the omp-not-found error when `shutil.which("omp")` is None.
_OMP_INSTALL_URL = "https://ohmy.pi"
...
def unwrap_omp(port: int, no_stop_proxy: bool) -> None:
    ...
    * Remove the Headroom proxy extension from ``~/.omp/agent/extensions/``.
